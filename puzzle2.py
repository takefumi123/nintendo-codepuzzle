class SimpleBars():
    ...
    def next(self):
        ...
        answer.append(self.__get_token(self.s[-1], self.s[0], self.s[1]))
        for i in range(1, len(self.s) - 1):
            answer.append(self.__get_token(self.s[i - 1], self.s[i], self.s[i + 1]))
        answer.append(self.__get_token(self.s[len(self.s) - 2], self.s[len(self.s) - 1], self.s[0]))

        self.s = answer
        return "".join(answer)

    def __get_token(self, a, b, c):
        print(a + " " + b + " " + c)
